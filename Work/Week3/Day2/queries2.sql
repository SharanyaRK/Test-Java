...
SELECT `marks`.`id`,`students`.`name`,`subject_id`,`year`,`quarterly`,`half_yearly`,`annual`,`year`,`grade` 
	FROM `marks` 
	JOIN `students` ON `students`.`id` = `marks`.`student_id`  
	WHERE `annual` IS NULL group by `year`;
...
SELECT `name`,`subject_id`,`year` 
	FROM `marks` JOIN `students` ON `students`.`id` = `marks`.`student_id` 
...
SELECT `name`,`subject_id`,`year` 
	FROM `marks` JOIN `students` ON `students`.`id` = `marks`.`student_id` 
	WHERE `annual` IS NOT NULL or `quarterly` IS NOT NULL or `half_yearly` IS NOT NULL;
...
SELECT `name`,`subject_id`,`year`,`quarterly`,`half_yearly`,`annual` 
	FROM `marks` 
	JOIN `students` ON `students`.`id` = `marks`.`student_id` WHERE `annual` > 90 and `half_yearly` > 90 and `annual` > 90;
...
-- Format: student_id, subject_id, average, year
 ...
 SELECT `marks`.`student_id`,`subject_id`,((COALESCE(`quarterly`,0)+COALESCE(`half_yearly`,0)+COALESCE(`annual`,0))/3) as `average`,`year` 
	FROM `marks` 
	JOIN `students` ON `students`.`id` = `marks`.`student_id`;
...
-- Format: student_id, subject_id, average, year
 ...
 SELECT `marks`.`student_id`,`subject_id`,((COALESCE(`quarterly`,0)+COALESCE(`half_yearly`,0)+COALESCE(`annual`,0))/3) as `average`,`year` 
	FROM `marks` 
	JOIN `students` ON `students`.`id` = `marks`.`student_id` WHERE `year` in (2003,2004);